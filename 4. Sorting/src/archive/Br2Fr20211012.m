classdef Br2Fr < handle
    ...
    methods
        ...
        function folder = get.folder(obj)
            folder = obj.nsF.folders(obj.index);
        end
        ...
        %             folders = dir(obj.folder);
        %             folders = struct2table(folders);
        %             temp = folders.name(folders.isdir);
        %             foldersName = string(temp(3:end));
        %             csvNames = strcat(obj.folder,"/klusta-",foldersName,".csv");
        %         end
        
        function [validChan, Tall, fr, frName] = br2fr(obj)
            Tall = [];
            validChan = [];
            dirExist = checkExist(obj, "dat");
            fprintf("----------\nsave dat from ns6\n")
...
%                 fprintf("%s has already opened, pass\n", fileName)
            end
%             fprintf("----------\nsorting\n")
%             [~, csvNames] = obj.doKlusta(folderName);
%             % Path = strcat(savePath, "/", date, Monkey);
%             LeastFiringCount = 30000; % about 4 spikes per second
%             RefractoryPeriod = 90; % 3ms
%             fprintf("----------\ncalculate fr\n")
%             [validChan_, T, fr, frName] = obj.datTofr(csvNames, ...
%                 LeastFiringCount, RefractoryPeriod);
%             validChan = [validChan;validChan_]; %#ok<AGROW>
%             Tall = [Tall;T]; %#ok<AGROW>
        ...
        function brTodat(obj, fileName, folderName, channels)
            ...
            for i = 1:10
                c_ = matrix(i,:);
                clear NS6
                openNSx(char(file),'uv', char(sprintf('c:%d:%d', min(c_), max(c_))));
                if i == 1
                    MetaTags = NS6.MetaTags;
                    MetaTags.ChannelID = channels;
                    MetaTags.ChannelCount = length(obj.Channels);
                end
                for index_ = 1:16
                    if ismember(c_(index_), channels)
                        fprintf("channel %d \n", c_(index_))
                        datI = int16(NS6.Data(index_,:));
                        saveDat(datI, c_(index_), fileName)
                    end
                end
            end
            clear NS6
            rmpath(genpath('NPMK'))
            %% one directory for each file
            files = struct2table(dir("*.dat"));
            ...
            for i = 1:length(files)
                if ~exist(folderNames(i), 'dir')
                    mkdir(folderNames(i))
                    movefile(files(i), folderNames(i))
                end
            end
            if length(dir(folderName)) ~= length(validChannels)+2
            ...
            if ~exist(name_,"file")
                ...
                error("MetaTags file %s already exists\n", name_)
            end
        end
        
        function [folder, csvNames] = doKlusta(obj, folderName)
            [PRM, DatFiles] = createPRM(folderName);
            for i = 1:length(PRM)
                prmname = PRM(i);
                if ~exist(DatFiles(i), "file")
                    fprintf("sorting %s\n", prmname)
                    [status,cmdout] = unix(sprintf("klusta %s --output-dir ./", prmname));
                    if status
                        error(cmdout)
                    end
                else
                    fprintf("already sorting %s, pass\n", prmname)
                end
            end
            fprintf("read firing rate from kwik and save csv\n")
            [folder, csvNames] = obj.kwik2csv(folderName);
        end
        
        function [folder, csvNames] = kwik2csv(~,Path)
            foldersName = dirFolders(Path);
            folder = strcat(Path, "/", foldersName);
            csvNames = strcat(Path,"/klusta-",foldersName,".csv");
            for i = 1:length(foldersName)
                file = dir(strcat(Path, "/", foldersName(i), "/*.kwik"));
...
%                         fprintf("----- cluster %d has %.2f spikes within RP, pass -----\n", j, RPall)
        ...
        function archiveDat(obj)
            Path = obj.folder;
            folders = obj.archiveFolder;
            fprintf("archive dat %s\n", Path)
            if exist(Path,"dir")
                mkdir(folders);
                [status, msg] = copyfile(Path, folders);
                folder_ = obj.folder;
                if status
                    for index_ = 1:length(folder_)
                        rmdir(folder_(index_),"s");
                    end
                else
                    error(msg);
                end
            else
                fprintf("already archive dat\n")
            end
        end
        ...
        function dirExist = checkExist(obj, type)
            ...
            if strcmp(type,"dat")
                ...
                if sum(dirExist) == length(obj.Channels)
                    return
                else
                    dirExist = zeros(length(obj.Channels),length(obj.datFolder));
...
function saveDat(datI, channel, fileName)
...
function [PRM, DatFiles] = createPRM(folder)
allData = struct2table(dir(folder));
allData = string(allData.name(allData.isdir));
allData = allData(3:end);
allData = strcat(folder, "/", allData);
