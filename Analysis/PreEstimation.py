...
def _readAuxiliaryData():
    ...
    '''
    # Load data
    locs_df = readLocDistance(f"{data_path}/dij_distance_map.csv")
    adjacent_data = readAdjacentMap(f"{data_path}/adjacent_map.csv")
    adjacent_path = readAdjacentPath(f"{data_path}/dij_distance_map.csv")
...
def _individualEstimation(all_data, adjacent_data, locs_df, adjacent_path, reward_amount):
    ...
    for index in range(num_samples):
        ...
        if "fruit_pos" in all_data.columns.values:
            ...
            if isinstance(each.fruitPos, list):
                fruit_pos = each.fruitPos[0]
            else:
