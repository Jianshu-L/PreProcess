...
def _readData(filename):
    ...
    # -----
    filename_list = df.DayTrial.unique()
    selected_data = pd.concat([df[df.DayTrial == i] for i in filename_list]).reset_index(drop=True)
    df = selected_data
    # -----------------
    # Drop some columns
...
if __name__ == '__main__':
    ...
    config = {
        "filename": f"{save_base}{file_name}-with_Q.pkl",
        "save_base": save_base
    }
